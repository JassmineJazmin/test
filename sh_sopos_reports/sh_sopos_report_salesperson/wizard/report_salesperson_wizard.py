...
class sale_pos_report_salesperson_xls(models.Model):
...
class sh_sale_pos_report_salesperson_wizard(models.TransientModel):
    ...
    def default_get(self, fields):
        rec = super(sh_sale_pos_report_salesperson_wizard, self).default_get(fields)
        search_users = self.env["res.users"].sudo().search([('company_id','in',self.env.context.get('allowed_company_ids',False))])
    ...
    def print_xls_report(self):
        ...
        worksheet = workbook.add_sheet('Sale and POS Report by Sales Person', bold_center)
        worksheet.write_merge(0, 1, 0, 5, 'Sale and POS Report by Sales Person', heading_format)
        left = xlwt.easyxf('align: horiz center;font:bold True')
        worksheet.write_merge(2, 2, 0, 5, str(self.date_start) + " to " + str(self.date_end), bold)
        ...
        for user_id in self.user_ids:
            row = row + 2
            worksheet.write_merge(row, row, 0, 5, "Sales Person: " + user_id.name, bold_center)
            row = row + 2
            ...
            if self.state == 'all':
                ...
                for sale_order in self.env['sale.order'].sudo().search(domain):
                    ...
                    worksheet.write(row, 1, str(sale_order.date_order))
                ...
                for pos_order in self.env['pos.order'].sudo().search(domain):
                    ...
                    worksheet.write(row, 1, str(pos_order.date_order))
                ...
                for sale_order in self.env['sale.order'].sudo().search(domain):
                    ...
                    if sale_order.invoice_ids:
                        for invoice_id in sale_order.invoice_ids.filtered(lambda inv: inv.state not in ['cancel', 'draft']):
                            ...
                            total_due_amount += invoice_id.amount_residual_signed
                    worksheet.write(row, 0, sale_order.name)
                    worksheet.write(row, 1, sale_order.date_order)
                    worksheet.write(row, 2, sale_order.partner_id.name)
                    worksheet.write(row, 3, sale_order.amount_total)
                    worksheet.write(row, 4, sum_of_invoice_amount)
                    worksheet.write(row, 5, sum_of_due_amount)

...
class Top_SOPOS_Customer_excel_extended(models.Model):
...
class sh_sopos_top_customer_wizard(models.TransientModel):
    ...
    date_compare_from = fields.Date(string='Compare From Date')
    date_compare_to = fields.Date(string='Compare To Date', default=fields.Date.today)
    
    no_of_top_item = fields.Integer(string='No of Items', required=True, default=10)
    ...
    company_ids = fields.Many2many('res.company',string="Company",default=default_company_ids)
    ...
    def _check_compare_from_to_dates(self):
        if self.filtered(lambda c: c.date_compare_to and c.date_compare_from and c.date_compare_from > c.date_compare_to):
            raise ValidationError(_('compare from date must be less than compare to date.'))  
    ...
    def print_top_customer_xls_report(self,):
        ...
        bold_center = xlwt.easyxf('font:bold True;pattern: pattern solid, fore_colour gray25;align: horiz center')
...
#       finally update data dictionary
        ...
        if self.type=='basic':
            row = 1
            worksheet = workbook.add_sheet(u'Top SO and POS Customers',cell_overwrite_ok=True)
            worksheet.write_merge(0, 1, 0, 2, 'Top SO and POS Customers',heading_format)
            worksheet.write(3,0, 'Date From: ',bold)
            worksheet.write(3,1, str(data['date_from'])) 
              
            worksheet.write(4,0, 'Date To: ',bold)
            worksheet.write(4,1, str(data['date_to']))
            ...
            row = 1
            worksheet = workbook.add_sheet(u'Top SO and POS Customers',cell_overwrite_ok=True)
            worksheet.write_merge(0, 1, 0, 6, 'Top SO and POS Customers',heading_format)    
            worksheet.write(3,0, 'Date From: ',bold)
            worksheet.write(3,1, str(data['date_from'])) 
            worksheet.write(4,0, 'Date To: ',bold)
            worksheet.write(4,1, str(data['date_to']))
            worksheet.write(3,5, 'Compare From Date: ',bold)
            worksheet.write(3,6, str(data['date_compare_from'])) 
               
            worksheet.write(4,5, 'Compare To Date: ',bold)
            worksheet.write(4,6, str(data['date_compare_to']))
            row = 7
            worksheet.col(0).width = int(25*260)
            worksheet.col(1).width = int(25*260)    
            worksheet.col(2).width = int(14*260) 
            ...
            for j in range(len(final_compare_partner_list)):
                ...
                worksheet.write(row, 6, final_compare_partner_amount_list[j], left)
                row=row+1
            row=row+2
            worksheet.write_merge(row,row, 0,2, 'New Customers',bold_center)
            worksheet.write_merge(row,row, 4,6, 'Lost Customers',bold_center)
            row=row+1
            start_row = row

...
class Sale_POS_Detail_Excel_Extended(models.Model):
...
class sh_sale_pos_details_report_wizard(models.TransientModel):
    ...
    def default_get(self, fields):
        rec = super(sh_sale_pos_details_report_wizard, self).default_get(fields)
    ...
    def print_report(self):
        ...
        data = {'date_start': self.start_date, 'date_stop': self.end_date,'team_ids': self.team_ids.ids, 'company_ids':self.company_ids.ids,'config_ids':self.config_ids.ids,'state': self.state}        
    ...
    def print_sale_pos_detail_xls_report(self):
        ...
        bold_center = xlwt.easyxf('font:height 225,bold True;pattern: pattern solid, fore_colour gray25;align: horiz center')
        ...
        worksheet.write_merge(2, 2, 0, 3, date_start + " to " + date_stop, center) 
        total = 0.0
        products_sold = {}
        taxes = {}
        invoice_id_list = []
...
#             currency = order.session_id.currency_id
            ...
            if order.invoice_ids:
                f_invoices = order.invoice_ids.filtered(lambda inv: inv.state not in ['draft', 'cancel'])
                if f_invoices:
        ...
        if invoice_id_list and search_journals:
            for journal in search_journals:
                domain = [ 
                    ...
                    ("invoice_ids", "in", invoice_id_list),
                ...
                journal_wise_total_payment_list.append({"name" : journal.name, "total" : paid_total})
...
#             currency = order.session_id.currency_id
            ...
            if pos_order.account_move:
                f_invoices = pos_order.account_move.filtered(lambda inv: inv.state not in ['draft', 'cancel'])
                if f_invoices:
                    invoice_id_list += f_invoices.ids
        ...
        if invoice_id_list and search_pos_journals:
            for pos_journal in search_pos_journals:
                ...
                journal_wise_total_payment_list.append({"name" : pos_journal.name, "total" : paid_total})
        ...
        for rec1 in list2:
            ...
            worksheet.write_merge(row, row, 2, 3, str(float_number), bold_right)
            row += 1
        row += 1    
        list3 = var.get("taxes")
